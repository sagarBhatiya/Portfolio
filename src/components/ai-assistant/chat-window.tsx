...
export default function ChatWindow({
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
      ...
      {messages.length === 0 && (
        <div className="max-w-2xl mx-auto my-auto flex flex-col items-center justify-center text-center space-y-6 py-10">
          <div className="w-16 h-16 rounded-2xl bg-[#10a37f]/20 border border-[#10a37f]/40 flex items-center justify-center text-[#10a37f] shadow-xl shadow-[#10a37f]/10 animate-bounce">
            <Bot size={32} />
          </div>

          <div className="space-y-2">
            <h2 className="text-xl font-bold text-white tracking-tight flex items-center justify-center gap-2">
              Chat with {candidateName}'s AI Assistant
            ...
            {SUGGESTION_CARDS.map((card, idx) => {
              ...
              return (
                <button
                  ...
                  className="p-3.5 rounded-xl bg-[#202123] border border-white/10 hover:border-[#10a37f]/50 hover:bg-white/[0.04] text-left transition group flex flex-col justify-between gap-2"
                >
      ...
      {messages.map((msg, index) => {
        ...
        return (
          ...
          >
            {!isUser && (
              <div className="w-8 h-8 rounded-full bg-[#10a37f]/20 border border-[#10a37f]/40 flex items-center justify-center text-[#10a37f] shrink-0 mt-1">
                <Bot size={18} />
              </div>
            )}
            ...
            >
              ...
              {!isUser && msg.content && (
                <div className="flex items-center justify-between pt-2 mt-2 border-t border-white/5 text-[11px] text-zinc-500">
            ...
            {isUser && (
              <div className="w-8 h-8 rounded-full bg-zinc-700 flex items-center justify-center text-white shrink-0 mt-1 text-xs font-semibold">
                <User size={16} />
              </div>
            )}
          </div>
